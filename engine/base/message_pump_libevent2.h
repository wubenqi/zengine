...
#ifndef BASE_MESSAGE_PUMP_LIBEVENT2_H_
#define BASE_MESSAGE_PUMP_LIBEVENT2_H_
...
class BASE_EXPORT MessagePumpLibevent2 : public MessagePump {
 public:
  class IOObserver {
   public:
    IOObserver() {}
  ...
  class FileDescriptorWatcher {
   ...
   private:
    friend class MessagePumpLibevent2;
    friend class MessagePumpLibeventTest;

    // Called by MessagePumpLibevent2, ownership of |e| is transferred to this
    // object.
    void Init(event* e);

    // Used by MessagePumpLibevent2 to take ownership of event_.
    event* ReleaseEvent();

    void set_pump(MessagePumpLibevent2* pump) { pump_ = pump; }
    MessagePumpLibevent2* pump() { return pump_; }

    void set_watcher(Watcher* watcher) { watcher_ = watcher; }

    void OnFileCanReadWithoutBlocking(int fd, MessagePumpLibevent2* pump);
    void OnFileCanWriteWithoutBlocking(int fd, MessagePumpLibevent2* pump);

    event* event_;
    MessagePumpLibevent2* pump_;
    Watcher* watcher_;
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(MessagePumpLibevent2);
};
