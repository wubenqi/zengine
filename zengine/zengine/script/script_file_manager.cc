...
void ScriptFileManager::Initialize(const std::vector<FilePath>& file_paths) {
  // 保证只初始化一次
  if (!watcher_file_paths_.empty()) return;

  for (size_t i=0; i<file_paths.size(); ++i) {
    // 是目录
    if (file_util::DirectoryExists(file_paths[i])) {
      file_util::FileEnumerator enumerator(file_paths[i], true,
          FILES_AND_DIRECTORIES,
          FILE_PATH_LITERAL("*"));

      for (FilePath file2 = enumerator.Next(); !file2.value().empty(); file2 = enumerator.Next()) {
        FilePath file;
        if (!file_util::NormalizeFilePath(file2, &file)) {
          file = file2;
        }

        if (file.Extension() == FILE_PATH_LITERAL(".lua")) {
          file_util::NormalizeFilePath(file, &file);
          // 保证只加载一次
    ...
    } else if (file_util::PathExists(file_paths[i])) {
      FilePath file;
      if (!file_util::NormalizeFilePath(file_paths[i], &file)) {
        file = file_paths[i];
      }
      // = file_paths[i];
      if (file.Extension() == FILE_PATH_LITERAL(".lua")) {
        ...
        if (script_datas_.find(file) == script_datas_.end()) {
          ...
          if (file_util::GetFileInfo(file, &file_info)) {
            ScriptFileData script_fild_data;
            base::PlatformFileInfo file_info;
            bool file_exists = file_util::GetFileInfo(file, &file_info);
